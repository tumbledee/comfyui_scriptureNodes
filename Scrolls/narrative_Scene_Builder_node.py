...
class narrative_Scene_Builder:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "scene_id": ("STRING", {"default": "scene_01", "multiline": False}),
            },
            "optional": {
                # ── Core scene fields ──────────────────────────────────────
                "scene": ("STRING", {"default": "", "multiline": True,
                    "tooltip": "Location, environment, atmosphere. Supports __wildcards__"}),
                ...
                "loras": ("STRING", {"default": "", "multiline": True,
                    "tooltip": "One per line or comma-separated: lora_name:weight  e.g.  detail_tweaker:0.7"}),
                ...
                "save_file_path": ("STRING", {"default": "",
                    "tooltip": "Full path to save scene JSON. Leave empty to skip."}),
                "load_file_path": ("STRING", {"default": "",
                    "tooltip": "Full path to load scene JSON. Overrides all inputs if set."}),
            },
        }

    RETURN_TYPES = ("SCENE", "STRING")
    RETURN_NAMES = ("scene", "debug_preview")
    ...
    def build_scene(
        ...
        auto_categories = ["scene", "lighting", "camera", "vfx"]
    ...
    def _render_debug(d: dict) -> str:
        sep = "─" * 48
        lines = [
            f"╔ SCENE BUILDER DEBUG  [{d.get('scene_id', '?')}]",
            sep,
...
# ── Registration ──────────────────────────────────────────────────────────────
NODE_CLASS_MAPPINGS = {
    "narrative_Scene_Builder_node": narrative_Scene_Builder,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "narrative_Scene_Builder_node": "narrative Scene Builder 🎬",
}
